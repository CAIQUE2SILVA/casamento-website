...
Cada componente segue o padrão de arquivos separados:
...
Ordem do `@Component`: `selector` → `standalone` → `imports` → `templateUrl` → `styleUrls`.
...
- Specs de componentes usam `provideHttpClient()` quando há dependência de serviços com `HttpClient`.
