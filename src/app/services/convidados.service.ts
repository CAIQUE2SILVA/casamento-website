...
export class ConvidadosService {
  ...
  private convidados: any[] = [
    ...
    { id: 2, nome: 'Maria Oliveira', email: 'maria@exemplo.com', confirmado: true }
  ];
  ...
  async addConvidado(convidado: any) {
    const novoId = this.convidados.length > 0
  ...
  getEstatisticas(): Promise<EstatisticasConvidados> {
    ...
    const convitesEnviados = this.convidados.filter(c => c.conviteEnviado).length;

    // Total de pessoas (convidados principais + acompanhantes)
    const totalPessoas = this.convidados.reduce((sum, c) => sum + 1 + c.acompanhantes.length, 0);

    // Total de pessoas confirmadas
    const totalConfirmados = this.convidados.reduce((sum, c) => {
      const acompanhantesConfirmados = c.acompanhantes.filter((a: any) => a.confirmado).length;
